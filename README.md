...
```
lan-beacon/
├── android/         # Android Gradle 工程（完整：settings.gradle.kts / gradlew / build-logic）
│   └── lib/          # 唯一发布模块，Kotlin 源码位于此
├── windows/         # 预留：桌面探测端（C# / Rust）
├── macos/           # 预留：桌面探测端（Swift / Rust）
├── protocol/        # 预留：三端共享的协议规范文档
└── Makefile         # 顶层入口，<平台>-<动作> 命名（make android-assemble）
```

顶层不含 Gradle wrapper 等平台特定资产，IDE 请以 `android/` 作为 Android Studio / IntelliJ 的工程打开点。

> 备注：如果使用 JitPack 发布，由于根目录不再是 Gradle 工程，需在仓库根提供 `jitpack.yml` 让 JitPack 进入 `android/` 后再构建（首次发版时一同加）。
