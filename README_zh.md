...
我在公司电脑上有一个桌面应用，里面有个「管家」功能——可以远程控制家里的 AI agent 干活。出于安全考虑，只有我本人在工位时才应该能使用管家。lan-beacon 的解决方式是：手机后台广播一个 WiFi 心跳，桌面端检测到手机在同一局域网时才解锁管家——一个轻量的「物理在场证明」，不需要密码或生物识别。
...
- [x] 0.2：可选 Token 鉴权（应对不可信局域网场景）
- [x] 0.2：mDNS TXT 记录承载更多元信息（设备名、能力声明）
- [x] 0.3：可插拔路由（让集成方扩展 healthz 之外的端点）
- [x] 0.3：IPv6 支持
- [x] 0.4：桌面端 KMP Scanner SDK（Kotlin Multiplatform，JVM target）
